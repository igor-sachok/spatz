...
def rand_llr_pair_generator(
    ...
    equal_prob    : probability of forcing b = a (the a == b path, returns 0).
    ...
    # Equal path: b = a, so the difference a - b is exactly zero. Applied last so
    # it wins, mirroring the priority of tackle_special_sums for subtraction.
    if equal_prob > 0:
        mask = np.random.rand(size) < equal_prob
        b[mask] = a[mask]
 
    return a.astype(np.int8), b.astype(np.int8)
 
 
def var_to_check_golden(a, b):
    """Reference model of compute_var_to_check_msgs (v2c = a - b), elementwise."""
    a = a.astype(np.int16)
    b = b.astype(np.int16)

    tmp = a - b
    # Finite difference saturates to +/- LLR_MAX (operator+=, NOT promotion).
    out = np.where(np.abs(tmp) > LLR_MAX, np.sign(tmp) * LLR_MAX, tmp)

    # tackle_special_sums(a, -b), in reverse priority order.
    out = np.where(np.abs(b) == LLR_INFTY, -b, out)   # isinf(b) -> -b
    out = np.where(np.abs(a) == LLR_INFTY,  a, out)    # isinf(a) ->  a
    out = np.where(a == b, 0, out)                     # a == b   ->  0
...
def main():
    ...
    equal_prob = param.get("equal_prob", 0.0)
    ...
    if verbose:
        golden = var_to_check_golden(var_to_check, check_to_var)
        n_zero = int(np.sum(var_to_check == check_to_var))          # a == b
        n_inf = int(
            np.sum(
        ...
        n_sat = int(
            np.sum(
                np.abs(
                    var_to_check.astype(np.int16) - check_to_var.astype(np.int16)  # a - b
                )
                > LLR_MAX
            )
        )
