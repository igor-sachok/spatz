...
int main() {
 ...
 // if (cid == 0)
  ...
  // Calculate dotp
  fdotp_cf_v32b(dotp_A_dram, dotp_B_dram, &result_re, &result_im);
...
//  }
  ...
  if (cid == 0) {
    long unsigned int performance = 1000 * 2 * dotp_l.M / (timer1+timer2);
    ...
    printf("The loading to vector registers took %u cycles.\n", timer1);
    printf("The vectoral computations took %u cycles.\n", timer2);
    printf("The performance is %ld OP/1000cycle (%ld%%o utilization).\n",
           performance, utilization);
  }

  if (cid == 0)
    if (fp_check(&result_re, &result_im, dotp_A_dram, dotp_B_dram)) {
      return -1;
    }
