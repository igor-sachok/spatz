...
int main() {
  ...
  if (cid == 0) {
    ...
    for (unsigned int i = 0; i < 2 * NFFT; i = i + 2) {
      if (fp_check(buffer[i], gold_out_dram[i])) {
        printf("Error: Index %d -> Result = %f, Expected = %f\n", i,
               (float)buffer[i], (float)gold_out_dram[i]);
      }
    }

    // Verify the imac part
    for (unsigned int i = 1; i < 2 * NFFT; i = i + 2) {
      if (fp_check(buffer[i], gold_out_dram[i])) {
        printf("Error: Index %d -> Result = %f, Expected = %f\n", i,
               (float)buffer[i], (float)gold_out_dram[i]);
      }
